...
topic and :func:`suggest_topics` matches those keywords against a story's
title/summary. No machine-learning or external calls are involved, so tagging
...
def suggest_topics(title: str, summary: str = "") -> list[str]:
    ...
    same suggestions. Pure function — performs no database access.
    """
...
def _get_topic(session, slug: str) -> Topic:
    ...
    topic = session.scalars(
        select(Topic).where(Topic.slug == name)
    ).first()
    if topic is None:
...
def get_topic(session, slug: str) -> dict:
    ...
    if topic.parent_id is None:
        ...
        related = session.scalars(
            select(Topic.slug)
            .where(
                Topic.parent_id == topic.parent_id,
                Topic.id != topic.id,
            )
            .order_by(Topic.slug)
        ).all()
...
def auto_tag_story(session, story_id: int) -> dict:
    """Auto-tag *story_id* from its title/summary via keyword matching.

    Delegates the slug selection to :func:`suggest_topics` and then reuses
    :func:`tag_story` so auto- and manual tagging share the same idempotent
    write path. A story whose text matches nothing is left untagged (a no-op).
    ...
    if story is None:
        raise TopicError(f"story {story_id} does not exist", not_found=True)
    slugs = suggest_topics(story.title, story.summary)
    if not slugs:
...
def auto_tag_all(session) -> dict:
    ...
    for story_id in session.scalars(select(Story.id)).all():
        result = auto_tag_story(session, story_id)
        if result["topics"]:
            tagged += 1
...
def _order_stories(stmt, sort: str):
    ...
    if sort == "score":
        return stmt.order_by(Story.score.desc(), Story.id.desc())
    return stmt.order_by(Story.created_at.desc(), Story.id.desc())
...
def topic_stories(
    ...
    *sort* is ``"recency"`` (newest ``created_at`` first, the default) or
    ``"score"`` (highest score first); any other value raises
    ...
    stories = session.scalars(
        _order_stories(base, sort).limit(limit)
    ).all()
    return {
        "slug": topic.slug,
        "sort": sort,
